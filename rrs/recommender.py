        ...
        for u in range(n_users):
            for m in range(n_items):
                ...
                if np.isfinite(ratings_mat[u, m]):
                    ...
                    err = ratings_mat[u, m] - \
                        np.dot(user_mat[u, :], item_mat[:, m])
                    # Keep track of the total sum of squared errors for the matrix
                    sse_accum += err**2
                    # update the values in each matrix in the direction of the gradient
                    user_mat[u, :] = user_mat[u, :] + \
                        learning_rate * 2 * err * item_mat[:, m].T
                    item_mat[:, m] = item_mat[:, m] + \
                        learning_rate * 2 * err * user_mat[u, :].T
...
class Recommender():
    ...
    def fit(self, latent_features=10, learning_rate=0.0001, iters=100, print_every=False):
        ...
        self.user_mat, self.item_mat = _fit(
            np.array(self.ratings_mat), latent_features, learning_rate, iters, print_every)

        self.user_df = pd.DataFrame(self.user_mat, index=self.ratings_mat.index, columns=[
                                    'k'+str(i+1) for i in range(latent_features)])
        self.item_df = pd.DataFrame(self.item_mat, columns=self.ratings_mat.columns, index=[
                                    'k'+str(i+1) for i in range(latent_features)])

        # Create prediction matrix and clip to min-max rating
        self.ratings_mat_pred = pd.DataFrame(np.dot(self.user_mat, self.item_mat), index=self.ratings_mat.index, columns=self.ratings_mat.columns) \
    ...
    def set_user_item_matrix(self, df, user_id_col_name, item_id_col_name, rating_col_name):
        ...
        Returns:
            ratings_mat - (pd.Dataframe) the user-item matrix (users as rows
                                      , items as cols, ratings as values)
        '''
        self.ratings_mat = self.get_user_item_matrix(
            df, user_id_col_name, item_id_col_name, rating_col_name)
        self.reviews_df = df
    ...
    def compare_train_to_predictions(self, test_df, ratings_mat_pred=None, plot=True):
        ...
        ratings_mat_test = self.get_user_item_matrix(
            test_df, self.user_id_col_name, self.item_id_col_name, self.rating_col_name)

        # Get users in both train and test
        shared_users = set(self.ratings_mat.index).intersection(
            ratings_mat_test.index)
        # Get items in both train and test
        shared_items = set(self.ratings_mat.columns).intersection(
            ratings_mat_test.columns)
        ...
        test = ratings_mat_test_cropped.fillna(
            -1).applymap(lambda y: str(round(y)))
        ...
        if plot:
            ...
            ax[1].hist(test_list, density=True, alpha=.5,
                       label='actual', bins=range(8))
            ax[1].hist(pred_list, density=True, alpha=.5,
                       label='predicted', bins=range(8))
    ...
    def plot_locations(self, recomms_df, business_id, item_name_col, latitude_name, longitude_name, info=None, search_string='', icon='cutlery'):
        ...
        center_location = recomms_df[latitude_name].mean(
        ), recomms_df[longitude_name].mean()
        ...
        for idx, row in recomms_df.iterrows():
            if not math.isnan(row[longitude_name]) and not math.isnan(row[latitude_name]):
                ...
                mc.add_child(Marker([row[latitude_name], row[longitude_name]], popup=popup, icon=folium.Icon(color=color, icon=icon, prefix='fa', icon_color='white')
                                    )
                             )
    ...
    def get_top_reviewed_items(self, min_number_of_reviews=0):
        ...
        '''
        mean_ratings = self.reviews_df[[self.user_id_col_name, self.item_id_col_name, self.rating_col_name]]      \
